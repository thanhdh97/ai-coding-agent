...
### 3. Gỡ theo dõi khỏi Git
Ngay sau khi cập nhật thành công, hãy gỡ các thư mục này khỏi bộ nhớ đệm (cache) của Git. Thao tác này đảm bảo khi bạn gõ `git commit`, các file của Agent sẽ không bị commit lẫn vào mã nguồn dự án gốc của bạn:
```bash
git rm --cached -r .agents .gitmodules
```
